...
__global__ void gemm_block(float *A, float *B, float *C)
{
    ...
    for (int sub = 0; sub < N / BLOCK; ++sub)
    {
        A_shared[local_row][local_col] = A[row * N + sub * BLOCK + local_col];
        B_shared[local_row][local_col] = B[(sub * BLOCK + local_row) * N + col];
        __syncthreads();
...
int main()
{
    ...
    gemm_block<<<gridsize, blocksize>>>(cu_A, cu_B, cu_C);
